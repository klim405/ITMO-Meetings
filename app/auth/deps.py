...
class UserInfo:
    ...
    async def get_model(self, db_session: AsyncSession) -> User:
        return await get_or_404(User, db_session, id=self.id)
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)]):
    ...
    async with make_async_session() as db_session:
        try:
            ...
            raise token_exception
        user = await User.get(db_session, id=token_data.user_id)
        if user is None:
            raise token_exception
